...
interface ILearningHistoryContent {
  ...
  contentType: string
  primaryCategory?: string // need to remove ?
  pageState: number
  loading: boolean
...
export class LearningHistoryComponent implements OnInit {
  ...
  selectedTabIndex = 0
  contentTypes = ['Program', 'course', 'collection', 'resource', 'certification']
  pageSize = 10
  loadingContent = true
  ...
  ngOnInit() {
    ...
    if (this.isClient) {
      ...
      this.route.data.subscribe(async data => {
        ...
        this.lhContent[0].primaryCategory = 'Program'
  ...
  getUserProgress(content: ILearningHistoryContent) {
    ...
    if (content.contentType !== 'certification') {
      if (content.primaryCategory === NsContent.EPrimaryCategory.MODULE) {
        this.pageSize = 20
      } else if (content.primaryCategory === NsContent.EPrimaryCategory.RESOURCE) {
        this.pageSize = 40
      } else {
        this.pageSize = 10
      }
