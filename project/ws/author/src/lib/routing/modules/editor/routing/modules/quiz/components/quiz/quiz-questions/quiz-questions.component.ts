...
export class QuizQusetionsComponent implements OnInit, OnDestroy {
  ...
  quizContent!: NSContent.IContentMeta
      ...
      if (this.activateRoute.parent && this.activateRoute.parent.parent) {
        this.activateRoute.parent.parent.data.subscribe(v => {
          this.quizResolverSvc.getUpdatedData(v.contents[0].content.identifier).subscribe(newData => {
            this.quizContent = this.metaContentService.getOriginalMeta(this.metaContentService.currentContent)
            const fileData = ((this.quizContent.artifactUrl || this.quizContent.downloadUrl) ?
              this.quizResolverSvc.getJSON(this.generateUrl(this.quizContent.artifactUrl || this.quizContent.downloadUrl)) : of({} as any))
                  ...
                  if (v.contents[0].data) {
                    ...
                    this.quizResolverSvc.getUpdatedData(id).subscribe(updatedData => {
                      if (updatedData && updatedData[0]) {
                        ...
                        // need to arrange
                        this.canEditJson = this.quizResolverSvc.canEdit(this.quizContent)
                        this.resourceType = this.quizContent.categoryType || 'Assessment'
                        this.quizDuration = this.quizContent.duration || '300'
                        this.questionsArr =
  ...
  triggerSave(meta: NSContent.IContentMeta, id: string) {
    ...
    // meta.versionKey = this.allContents[0].versionKey
  ...
  uploadJson(array: any[], fileName: string) {
    ...
    const quizData = {
      ...
      timeLimit: parseInt(this.quizDuration + '', 10) || QUESTION_CONFIG.defaultDuration,
  ...
  checkEditCondition() {
    if (this.configService.userProfile && this.quizContent && this.configService.userProfile.userId === this.quizContent.createdBy) {
      if (this.quizContent.status.toLowerCase() === 'draft') {
        if (!this.quizContent.prevStatus) {
          return true
        }
        if (this.quizContent.prevStatus.toLowerCase() !== 'live' && this.quizContent.prevStatus.toLowerCase() !== 'review') {
          return true
        }
      }
    }
