...
export class EditorContentService {
  ...
  assessmentOriginalContent: { [key: string]: NSContent.IContentMeta } = {}
  ...
  public shownLiveEditOnce = false
  ...
  getOriginalMeta(id: string): NSContent.IContentMeta {
    const returningData = this.originalContent[id]
  ...
  resetOriginalMetaWithHierarchy(meta: any) {
    ...
    this.resetOriginalMeta(meta, meta.identifier)
  ...
  originalMetaWithHierarchyUpdate(meta: any): NSContent.IContentMeta {
    if (meta.primaryCategory === NSContent.EPrimaryCategoryType.Program
      || meta.primaryCategory === NSContent.EPrimaryCategoryType.Course) {
      meta.creatorContacts =
        (this.jsonVerify(meta.creatorContacts) ? JSON.parse(meta.creatorContacts) : []) as NSContent.IAuthorDetails[]
      meta.trackContacts =
        this.jsonVerify(meta.reviewer) ? JSON.parse(meta.reviewer) : []
      meta.creatorDetails =
        this.jsonVerify(meta.creatorDetails) ? JSON.parse(meta.creatorDetails) : []
      ...
      if (meta.children && meta.children.length > 0) {
        meta.children.forEach((element: any) => {
          this.originalMetaWithHierarchyUpdate(element)
        })
      } else {
        return meta
      }
  ...
  setReviewerToTrackContacts(id: string, reviewer: string) {
    this.originalContent[id].trackContacts = JSON.parse(reviewer)
  }
      ...
      parentData.children.forEach((element: any) => {
        ...
        if (element.children && element.children.length > 0) {
          parentData.children.forEach((subEle: any) => {
            if (subEle.primaryCategory === NSContent.EPrimaryCategoryType.Collection) {
