...
export class AuthAssessmentAddQuestionFormMetaComponent implements OnChanges {
  ...
  async saveSection() {
    ...
    if (this.questionContentForm.controls['primaryCategory'].value === NsContent.EPrimaryCategory.ASSESSMENT) {
      if (this.questionContentForm.controls['totalQuestions'].value === 0 ||
        this.questionContentForm.controls['maxQuestions'].value === 0) {
        this.showTosterMessage('valueCannotBeZero')
        return
      }
      if (
        this.questionContentForm.controls['totalQuestions'].value < 0 ||
        this.questionContentForm.controls['maxQuestions'].value < 0
      ) {
        this.showTosterMessage('invalidValues')
        return
      }
    }
    if (this.questionContentForm.controls['primaryCategory'].value === NsContent.EPrimaryCategory.FINALASSESSMENT) {
      if (this.questionContentForm.controls['totalQuestions'].value === 0 ||
        this.questionContentForm.controls['maxQuestions'].value === 0 ||
        this.questionContentForm.controls['minimumPassPercentage'].value === 0) {
        this.showTosterMessage('valueCannotBeZero')
        return
      }
      if (
      ...
      if (this.questionContentForm.controls['minimumPassPercentage'].value > 100) {
        this.showTosterMessage('invalidPercentageValue')
        return
      }
    }
    this.loaderService.changeLoad.next(true)
    ...
    switch (type) {
      case 'invalidPercentageValue':
        this.snackBar.openFromComponent(NotificationComponent, {
          data: {
            type: Notify.INVALID_PERCENTAGE_VALUE,
          },
      ...
      case 'invalidValues':
        ...
        break
      case 'valueCannotBeZero':
        this.snackBar.openFromComponent(NotificationComponent, {
          data: {
            type: Notify.VALUES_CANNOT_BE_ZERO,
          },
          duration: NOTIFICATION_TIME * 1000,
        })
        break
  ...
  questionTextChange() {
    if (this.questionType === 'fitb') {
      if (this.questionText.includes('-')) {
        this.fitbCount += 1
        this.questionText = this.questionText.split('-').join('_______________')
      }
    }
  }
