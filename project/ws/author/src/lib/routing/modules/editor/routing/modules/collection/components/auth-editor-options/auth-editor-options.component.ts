...
export class AuthEditorOptionsComponent implements OnInit {
  ...
  primaryCategoryTypes = NsContent.EPrimaryCategory
  ...
  ngOnInit() {
    ...
    const contentTypeConfig = childrenConfig[this.node.primaryCategory]
    ...
    if (this.node.parentId) {
      ...
      if (
        parentType &&
        childrenConfig[parentType.primaryCategory === NsContent.EPrimaryCategory.PROGRAM ? parentType.primaryCategory
          : parentType.contentType] &&
        this.node.editable &&
        !parentContent.isContentEditingDisabled &&
        this.accessService.hasAccess(parentContent) &&
        childrenConfig[parentType.primaryCategory === NsContent.EPrimaryCategory.PROGRAM ? parentType.primaryCategory
          : parentType.contentType].allowCreation
      ) {
  ...
  formChildren(contentTypeConfig: any, currentDepth: number): ICustomCreateEntity[] {
    ...
    topLevel.forEach(v => {
      if (
        ...
        currentDepth + (DEPTH_RUE[v.primaryCategory === NsContent.EPrimaryCategory.PROGRAM ? v.primaryCategory : v.contentType] || 1)
      ) {
        child.push(
    ...
    if (content.children) {
      content.children.forEach(v => {
        ...
        if (
          ...
          this.authInitService.collectionConfig.maxDepth >=
          currentDepth + (DEPTH_RUE[entity.primaryCategory === NsContent.EPrimaryCategory.PROGRAM ? entity.primaryCategory
