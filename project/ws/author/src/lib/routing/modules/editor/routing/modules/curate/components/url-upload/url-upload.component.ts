...
export class UrlUploadComponent implements OnInit {
  ...
  assignData(meta: NSContent.IContentMeta) {
    ...
    this.urlUploadForm.controls.mimeType.setValue(meta.mimeType || 'application/html') // text/x-url
  ...
  submit() {
    if (this.urlUploadForm.controls.artifactUrl.value) {
      if (this.urlUploadForm.controls.artifactUrl.value.indexOf('https://www.youtube.com/embed/') !== -1
        || this.urlUploadForm.controls.artifactUrl.value.indexOf(`${environment.karmYogi}surveys/`) !== -1) {
