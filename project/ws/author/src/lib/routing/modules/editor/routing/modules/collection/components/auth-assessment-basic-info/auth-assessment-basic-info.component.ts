...
export class AuthAssessmentBasicInfoComponent implements OnChanges {
  ...
  async assignData() {
    this.contentForm.setValue({
      ...
      expectedDuration: (this.metaData.expectedDuration) ? this.metaData.expectedDuration : 0,
      ...
      Object.keys(this.contentForm.value).forEach((element: any) => {
        if (this.contentForm.controls[element].value !== this.metaData[element]) {
          if (element === 'showTimer') {
            const formContentValue = (this.contentForm.controls[element].value) ? 'Yes' : 'No'
            if (this.metaData[element] === formContentValue) {
              flag += 1
            } else {
              editiedMeta[element as keyof any] = formContentValue
              if (formContentValue === 'No') {
                editiedMeta['expectedDuration'] = 0
              }
            }
          } else {
