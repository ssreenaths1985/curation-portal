...
export class AccessControlService {
  ...
  hasRole(role: string[]): boolean {
    ...
    role.forEach(v => {
      if (this.configService.userRoles && (this.configService.userRoles).has(v)) {
        returnValue = true
      }
    })
    return returnValue
  ...
  get userFUllName(): string {
    if (this.configService.userProfile) {
      return `${this.configService.userProfile.firstName || null} ${this.configService.userProfile.lastName || null}` || 'User'
    }
    return ''
  }
  get userName(): string {
  ...
  hasAccess(
    ...
    if (['Draft', 'Live'].indexOf(meta.status) > -1) {
      if (typeof (meta.creatorContacts) === 'string') {
        meta.creatorContacts = JSON.parse(meta.creatorContacts)
      }
    ...
    if ((meta.reviewStatus === 'Reviewed' && meta.status === 'Review') && this.hasRole(['content_publisher'])) {
      if (meta.publisherIDs && meta.publisherIDs.length) {
        meta.publisherIDs.forEach(v => {
          if (v === this.userId) {
            returnValue = true
          }
        })
  ...
  getCategory(content: NSContent.IContentMeta | ISearchContent): string {
    return content.primaryCategory || content.contentType
  }
  ...
  getCategoryType(content: NSContent.IContentMeta | ISearchContent): string {
    switch (this.getCategory(content)) {
      case 'Resource':
        return content.primaryCategory || content.resourceType || 'Learning Resource'
      case 'Collection':
      case 'CourseUnit':
        return content.primaryCategory || 'Course Unit'
      case 'Course':
  ...
  getIcon(content: NSContent.IContentMeta | ISearchContent): string {
    if (content.mimeType === MIME_TYPE.collection) {
      if (this.getCategory(content) === NSContent.EPrimaryCategoryType.KBoard) {
        return ICON_TYPE.kBoard
      }
      if (this.getCategory(content) === NSContent.EPrimaryCategoryType.Program) {
        return ICON_TYPE.program
      }
      if (this.getCategory(content) === NSContent.EPrimaryCategoryType.Course) {
        return ICON_TYPE.course
      }
      return ICON_TYPE.learningModule
    }
    if (content.mimeType === MIME_TYPE.html) {
      if (content.resourceType === NSContent.EPrimaryCategoryType.Certification) {
    ...
    if (content.mimeType === MIME_TYPE.quiz || content.mimeType === MIME_TYPE.quizV2) {
      if (this.getCategoryType(content) === NSContent.EPrimaryCategoryType.Assessment) {
    ...
    if (content.mimeType === MIME_TYPE.mp4 || content.mimeType === MIME_TYPE.video) {
